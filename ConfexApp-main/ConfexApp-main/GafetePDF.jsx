...
const watermarkText = "GAFETE DE IDENTIFICACIÓN";
const watermarkRows = 6;
const watermarkCols = 1;

for (let row = 0; row < watermarkRows; row++) {
  for (let col = 0; col < watermarkCols; col++) {
    watermarkPattern.push(
      <Text
        key={`wm-${row}-${col}`}
        style={{
          position: "absolute",
...
const GafetePDF = ({
  ...
  // Estado para la imagen de perfil (foto del usuario)
  const [fotoPerfilSrc, setFotoPerfilSrc] = React.useState(null);
  // Estado para la imagen del congreso (marca de agua)
  const [marcaAguaSrc, setMarcaAguaSrc] = React.useState(null);
  ...
  const getImageUrl = () => {
    ...
    if (congreso && congreso.idCongreso) {
      return `${BACKEND_URL}/User/fotoUsuario/${user.idUsuario}`;
    }
  ...
  React.useEffect(() => {
    const loadUserImage = async () => {
      ...
      if (url.startsWith("data:image")) {
        setFotoPerfilSrc(url);
      } else {
        setFotoPerfilSrc(await convertImageToBase64(url));
      }
    };
    loadUserImage();
    // eslint-disable-next-line
  }, [user, congreso, BACKEND_URL]);

  // Cargar la imagen del congreso como marca de agua
  React.useEffect(() => {
    const loadCongresoImage = async () => {
      if (!congresoLogoUrl) {
        setMarcaAguaSrc(null);
        return;
      }
      if (congresoLogoUrl.startsWith("data:image")) {
        setMarcaAguaSrc(congresoLogoUrl);
      } else if (congresoLogoUrl.startsWith("/")) {
        setMarcaAguaSrc(
          await convertImageToBase64(`${BACKEND_URL}${congresoLogoUrl}`)
        );
      } else if (congresoLogoUrl.startsWith("http")) {
        setMarcaAguaSrc(await convertImageToBase64(congresoLogoUrl));
      } else {
        setMarcaAguaSrc(null);
      }
    };
    loadCongresoImage();
  }, [congresoLogoUrl, BACKEND_URL]);
  ...
  // Mostrar loading mientras se carga la imagen del congreso
  if (!fotoPerfilSrc) {
    return (
      <Document>
